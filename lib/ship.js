...
(function () {
  ...
  Ship.prototype.goShip = function (impulse) {
    var dx = (Math.sin(this.theta) * 1);
    var dy = -(Math.cos(this.theta) * 1);
    var thruster = impulse === 1 ? THRUST / 6 : -REV_THRUST / 6;
    var facingVector = [dx, dy];

    this.vel[0] = Math.abs(this.vel[0]) <= MAX_SPEED ?  this.vel[0] + facingVector[0] * thruster : this.vel[0];
    this.vel[1] = Math.abs(this.vel[1]) <= MAX_SPEED ?  this.vel[1] + facingVector[1] * thruster : this.vel[0];
  };
